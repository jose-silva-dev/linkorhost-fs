    ...
    <section className="py-24 bg-[#0C0C17] w-full scroll-mt-24" id="plans-website">
      <div className="container px-4 mx-auto max-w-7xl">
        <div className="flex flex-col items-center mb-16">
          <h2 className="text-4xl font-bold tracking-tight text-white mb-4">Hospedagem de Site</h2>
          <p className="text-gray-400 text-lg max-w-2xl text-center">Obtenha mais com a Hospedagem de Site da Linkor</p>
        </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-10">
                <TabsList className="bg-gray-950 border border-[#1A1A35] p-1 h-12">
                  <TabsTrigger
                    value="cpanel"
                    className="data-[state=active]:bg-[#1A1A35] data-[state=active]:text-white text-gray-400 h-10 px-6 relative"
                  >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-gray-400 text-sm flex items-center gap-2">
                    Trienal
                    <span className="bg-[#F04339]/10 text-[#F04339] text-xs px-2 py-0.5 rounded-full">-40%</span>
                  </span>
                </div>
              </div>
    ...
    <div className="bg-gray-950 border rounded-lg transition-all duration-300 cursor-pointer border-[#F04339] shadow-[0_0_20px_rgba(240,67,57,0.15)]">
      <div className="p-6">
        <div className="flex justify-between items-start mb-6">
          ...
          {plan.recommended && (
            <div className="bg-[#F04339]/10 text-[#F04339] text-xs px-3 py-1.5 rounded-md font-medium">Recomendado</div>
          )}
        </div>
        ...
        <div className="space-y-3 mb-6">
          {plan.features.slice(0, isExpanded ? undefined : featuresLimit).map((feature, index) => (
            <div key={index} className="flex items-start gap-3">
              {feature.included ? (
                <CheckIcon
                  className={cn("w-4 h-4 mt-0.5", feature.highlight ? "text-[#F04339]" : "text-emerald-500")}
                />
              ) : (
              ...
              <span
                className={cn(
                  "text-sm",
                  feature.included ? (feature.highlight ? "text-white font-medium" : "text-gray-300") : "text-gray-500",
                )}
              >
                {feature.text}
                {feature.tooltip && (
